...
Usage:
  join-and-impute-vaccines-timeseries.R -o <path> --caseDeathRR <path> --vax <path> [--rejects <path>] [--writeRejects <path>] --metadata <path> [--writeMetadata <path>]
...
Options:
  ...
  --rejects <path>        Path to CSV storing [fips, code, reason] for excluded fips
  --writeRejects <path>   Where to write updated rejects file (appended to --rejects)
...
if (!is.null(args$rejects)) {
  ps("Loading rejects from {.file {args$rejects}}")
...
nearly_final <- left_join(caseDeathRR, vax_day_imp, 
                   by = c("date","fips")) %>%
  replace_na(list(vaccinated = 0))
pd()

cli_h1("Excluding all counties where we don't have {.code logor_vac} data")
final <- filter(
  nearly_final,
  !str_detect(fips, '^72') & !fips %in% c("02158", "46102")
)
...
if (!is.null(args$rejects)) {
  rejects <- bind_rows(
    rejects,
    tibble(
      fips = setdiff(unique(nearly_final$fips), unique(final$fips)),
      code = "NO_LOGOR",
      reason = "No log-odds information available for vaccines in this county"
    )
  )
}
